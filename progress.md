## 2026-09-23 - Task: 机器人一直在躲 Boss——会后退的 Boss 因此从来没被打过

### 诊断

平衡报告里腐蚀母株击杀率 79%、母巢暴君 94%，看起来又是一次难度差。先做那件已经救过一次场的事：**把失败的局拆开看是怎么失败的。**
...
母株 24 场一次都没杀死玩家。她不是更致命，她是没被打死。

给无头模拟加了个每步回调，把一场失败的局打印出来（seed 21 / juggernaut）：

```
t=240s boss=4600/4600 dist=574 wardens=0  lvl=24
t=300s boss=3914/4600 dist=307 wardens=7  lvl=32
t=360s boss=3398/4600 dist=306 wardens=4  lvl=42
t=410s boss=3192/4600 dist=300 wardens=17 lvl=47
```

47 级、六把满级武器，180 秒打掉 1408 血——**7.8 DPS**，距离稳定卡在 300px 附近。

两个各自合理的阈值锁死了：

- `enemyAI.ts`：母株在玩家进入 **260px** 时后退。
- `aiInput.ts`：机器人的威胁场把 Boss 权重记作 **×6**，作用半径 **230px**。

于是母株退多远，机器人就躲多远，谁也不越过那个环。暴君掩盖了这个问题——它是**追**着玩家打的，机器人想不想接近都会发生接触。**只有会后退的 Boss 才会把它暴露出来。**

所以要修的不是 Boss，是尺子：这个项目至今所有关于母株的数字，测的都是机器人的回避倾向，不是这场仗本身。

### 为什么这是第三次

`2026-09-13` 按「20% vs 61%」削过母株（召唤 7→9.5、狂暴 50%→35%），没用。
`2026-09-14` 拆开失败发现是 300 秒上限制造的伪指标，回退了削弱，把上限提到 600 秒。

但那一轮治的是症状。**即使放到 600 秒，当时仍有 3/20 超时**——那 3 局就是本轮这个 bug 在漏出来。机器人修好之后，**420 秒上限下超时归零（0/24）**，比 600 秒的变通做法更干净。

### What was done

- `BotTuning` 新增 `boss` 项（默认 4，与 `loot` 同量级：只影响取向，永远压不过威胁项）。Boss 距离 > `BOSS_ENGAGE`(200px) 时产生吸引，进入以内交还给威胁场——不然机器人会直接贴脸吃接触伤害。
- `boss: 0` 精确复现旧行为，两者可直接对比。
- 把诊断用的一次性 `probe` 回调提升为 `SimOptions` 的正式选项并写明理由：这次就是**读代码读不出来**的，两个阈值分散在两个文件里，只有打印出来才看得见。
- **没有改母株的任何数值。**

### A/B（24 种子 × 3 干员，配对）

`boss` 项在 Boss 出场前完全惰性，所以同一种子在两组里抽到的是同一个 Boss——56 对全部配上，无一例外，配对设计成立。

| Boss | 组 | 击杀率 | 超时 | 在 260px 内的时间 | 平均 DPS |
|---|---|---|---|---|---|
| 腐蚀母株 | `boss:0` | 79%（19/24） | 5 | 13% | 47.4 |
| 腐蚀母株 | `boss:4` | **100%（24/24）** | **0** | 80% | 147.6 |
| 母巢暴君 | `boss:0` | 94%（30/32） | 0 | 33% | 131.2 |
| 母巢暴君 | `boss:4` | 97%（31/32） | 0 | 90% | 261.6 |

母株 5 对全部由「没打完」翻为「击杀」，无一反向——McNemar 精确检验 **p = 0.031**，且方向与因果预测一致。

**暴君要说准确**：结果几乎没动（+1 杀，1 反向 2 正向，p = 1.0），但**行为动得很厉害**（贴近时间 33%→90%，DPS 翻倍）。这个修复不是母株专用的，只是暴君本来就在 94%，没有上升空间。拿「暴君没变」当作「只影响母株」是读错了。

### Testing

- `npx tsc --noEmit`、`npm run lint`、`npm test`（286 个测试，含确定性用例）全绿。
- 新增两个转向测试：一个断言**符号翻转的两边**——`boss:0` 必须朝远离 Boss 的方向走，`boss:4` 必须朝向它。所以它对旧行为是**失败**的，而不只是对新行为通过。另一个断言进入 200px 后吸引项交还威胁场。
- 换尺子之后整份平衡报告重跑（24 种子 × 3 干员 × 420s），结果与 A/B 完全一致：
...
**旧报告里所有与 Boss 相关的数字都是用坏掉的尺子量的，不能与新数据横向比较。**

顺带一个容易误读的地方：中位击杀 / 平均等级 / 金币全都**下降**了（如 juggernaut 4494 → 3844）。这不是变弱，是**局变短了**——胜利条件就是打死 Boss，以前打不死所以一直刷到 420 秒上限。攒的少了恰恰是因为赢得快了。

### 修完之后冒出来的新问题

Boss 击杀率现在是 97% / 100%，也就是说**Boss 战已经完全不是筛子了**：胜率基本等于「能不能活到 240 秒」。这不一定是坏事（机器人只是个下限参考，不是真人），但它意味着下一轮平衡的着力点在 Boss 之前的那段，而不是 Boss 本身。**先量，再改**——这里不再猜。

### 教训

`2026-09-14` 写下的那条「先问它是怎么失败的，再问哪个数值该调」是对的，而且正是它第二次救了场——两行 died/timeout 拆分又一次直接指到了根因。

这次补上一条更狠的：**当指标异常时，先怀疑测量装置，而不是被测对象。** 三次里有三次，问题都在尺子上——上限、上限、机器人——一次都不在 Boss 身上。

还有一条方法论：**任何"合理"的常量都可能和另一个文件里同样合理的常量锁死。** 260 和 230 单独看都没问题，是它们的关系制造了死局。读单个文件永远看不见这种 bug，只有把系统跑起来打印状态才能。

### 过程中的两次失误（都是真金白银的时间）

- 第一次基线跑到一半时我改了它已经加载的源文件，整组数据作废重来。**测量运行期间不碰 `src/`。**
- 重跑时用 `pkill -f "vite-node …"` 清理旧进程，这个模式**匹配到了它自己所在 shell 的命令行**，于是它杀掉了自己正要启动的那个任务（退出码 144），而 heredoc 都还没来得及写文件。

### Notes

- `src/sim/aiInput.ts`：新增 `BotTuning.boss`、`BOSS_ENGAGE`、`bossSeek()`。
- `src/sim/headless.ts`：`SimOptions.probe`。
- `tests/simHarness.test.ts`：两个转向测试 + 补 `boss` 字段。
- 回滚方式：把 `DEFAULT_BOT.boss` 设为 0 即可精确恢复旧行为，无需回退提交。

## 2026-09-16 - Task: 右摇杆只在按住时有效——瞄准没有保持

### 反馈

「触控应该左屏幕走动，右屏幕操控瞄准。」

左右分区本来就是这么实现的，所以先复现，不解释。**在 iframe 里测**（artifact 是嵌在 iframe 里跑的，之前只直接测过页面）：左拖角色位移正常，右拖瞄准向量和拖拽方向完全吻合。分区没问题。
...
**只要还在走，「移动方向」就永远排在「上次朝向」前面**，于是松开右摇杆的那一刻，刚刚瞄好的方向立刻被走位方向覆盖。右屏幕确实控制瞄准——但只在按住的那几百毫秒里。玩起来的感受就是「右边好像没用」，这和反馈完全对得上。

双摇杆射击的惯例是**瞄准保持**：松手后枪口停在那儿，直到你再次拨动。我当初把移动兜底排在前面，是为了单手也能玩，但没意识到它会吃掉双手玩家的输入。

### What was done

`resolveAim` 增加 `aimIsDeliberate`——这个玩家有没有碰过右摇杆：

- **没碰过（单手）**：枪口跟着脚走。行为不变。
- **碰过（双手）**：右摇杆拥有瞄准权，**松手后保持**，不再被移动方向覆盖。

`TouchControls` 记录 `aimUsed`，第一次启动右摇杆时置位。

### Testing

- `npx eslint .`、两个 TS 工程、`npm run i18n:audit`、`npm run build`：各自单独执行，退出码均为 0。
- `npm test`：284 个测试（`tests/stick.test.ts` 新增 3 条，其中一条直接盯住这个回归：「deliberate 玩家松手后仍在走动时，瞄准必须保持」）。
- **在 iframe 里用真触摸事件验证的三段式**：左拇指按住向右走 → 瞄准 (1,0)；右拇指向上拨 → 瞄准 (0,-1)；**只松开右拇指、左拇指继续按住**（`moving: true`）→ 瞄准仍是 (0,-1)。修之前第三步会弹回 (1,0)。

### 教训

**第一版测试没测到它，因为测试和实现是同一个误解。** 我写的是「松手后回落到上次朝向」，测的也是「站着不动时保持上次朝向」——而 bug 恰恰只在**移动中**松手才出现，那条路径一次都没被覆盖。

写测试的时候，两个兜底分支之间的**优先级**要单独测，不能只测每个分支自己能返回值。

顺带一个测试方法上的错：第一次验证时我用 CDP 的 `touchEnd` 传了错误的手指 id（它要的是**结束的**触点），结果松开的是移动拇指而不是瞄准拇指，`moving: false` 暴露了这一点。改对之后才是真的证明。

### Notes

- `src/input/stick.ts`、`src/input/touch.ts`、`src/input/provider.ts`、`tests/stick.test.ts`。
- 回滚方式：回退本任务对应提交。
